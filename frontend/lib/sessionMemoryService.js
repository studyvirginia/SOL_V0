...
export function buildLangChainSystemPrompt({
  ...
  const sections = [
    ...
    curriculumContext.mode === "notes" ? [
      "  showImage     — Notes mode only. Search for a contextual, openly-licensed image to visually anchor a concrete concept mid-explanation.",
      "                  Think of it like RoughNotation but spatial: use it ONCE per response, purposefully, after your first explanatory paragraph.",
      "                  Args: { query (1-3 concrete noun keywords, e.g. 'brass microscope'), lessonContext (1-sentence description of current lesson) }",
      "  showImage RULES (read before every call):",
      "    - MAXIMUM ONE showImage call per response. Never call it twice.",
      "    - ONLY for concrete physical objects or real places:",
      "        YES: 'roman aqueduct', 'plant cell diagram', 'civil war cannon', 'brass microscope'",
      "        NO:  'causes of the civil war', 'importance of photosynthesis', 'the industrial revolution'",
      "    - Use 1-3 keywords MAX. Short noun phrases only.",
      "    - Place it AFTER your first explanatory paragraph, never at the top.",
      "    - Never tell the student you are searching for an image. It should appear naturally.",
      "    - If the topic is entirely abstract (ideas, emotions, themes), do NOT call showImage.",
    ].join("\n") : null,
