 ...
 * Returns:  { desmosState: { expressions, viewport, showGrid, showAxes, degreeMode, polarMode, title } }
 */
...
export default async function handler(req, res) {
  ...
  // Sanitise equations: replace LaTeX function notation that Desmos can't parse
  const sanitized = equations.map(eq =>
    eq
      .replace(/\\lfloor\s*(.*?)\\rfloor/g, "floor($1)")
      .replace(/\\lceil\s*(.*?)\\rceil/g, "ceil($1)")
      .replace(/\\left\s*\|\s*(.*?)\s*\\right\s*\|/g, "abs($1)")
  ...
  const systemPrompt = buildDesmosPrompt({ type, equations: sanitized, label_points, course, notes, description });

  try {
    let model;
    if (config.provider === "openrouter") {
      model = createOpenAICompatible({ name: "openrouter", baseURL: "https://openrouter.ai/api/v1", apiKey: config.apiKey })(config.model);
    } else {
    ...
    const { text } = await generateText({
      model,
      system: systemPrompt,
      prompt: `Generate the Desmos graph state for the spec above.`,
      maxTokens: 1400,
      temperature: 0.05,
    });
    ...
    // One retry — the model occasionally truncates or wraps the JSON unexpectedly
    if (!desmosState) {
      console.warn("[desmos-generate] first attempt failed, retrying. Raw:", text.slice(0, 200));
      const { text: text2 } = await generateText({
        model,
        system: systemPrompt,
        prompt: `Generate the Desmos graph state for the spec above. Output ONLY the JSON code block, nothing else.`,
        maxTokens: 1400,
        temperature: 0.0,
      });
